...
from models import Unet_local
from models import vnet_l
import config
...
from models.dskip_loc import DenseNet
...
def main():
    ...
    # -------------init Seg---------------
    # model_S = Unet_local.UNet(1, [16, 32, 48, 64, 96], 1, net_mode='3d').cuda()
    # model_S = DenseNet(num_init_features=32, growth_rate=16, block_config=(4, 4, 4, 4), drop_rate=0.2, num_classes=1).to(device)
    model_S = Unet.UNet(1, [16, 32, 48, 64, 96], 1, net_mode='3d').cuda()
    # saved_state_dict = torch.load('./experments/unet8/latest_model.pth')
    # model_S.load_state_dict(saved_state_dict['net'])
    # model_S = vnet3d.VNet().cuda()
    # model_S = vnet_l.VNet().cuda()
    # model_S = transunet(vit).cuda()
    # --------------Loss---------------------------
    #criterion_S = nn.CrossEntropyLoss().cuda()
    criterion_1 = FocalLoss().cuda()   
    criterion_2 = DiceLoss().cuda()
    model_S.apply(weights_init.weights_init_kaiming)
    # setup optimizer
    ...
    trainloader = DataLoaderX(mri_data_train, batch_size=args.batch_train, shuffle=True)
    mri_data_val = H5Dataset(args.val_dataset_path, mode='val')
    valloader = DataLoaderX(mri_data_val, batch_size=4, shuffle=False)
